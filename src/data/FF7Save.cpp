...
FF7Save::FF7Save(QObject *parent) :
    QObject(parent)
    , Text(new FF7TEXT)
{
    for (int i = 0; i < 15; i++) {
    ...
    memcpy(&buffer_slot, &default_save, _slotSize);
}
...
bool FF7Save::loadFile(const QString &fileName)
{
    ...
    for (int i = 0; i < SG_SLOT_NUMBER; i++) {
        ...
        setSlotFF7Data(i, file.read(_slotSize));
    ...
    case FORMAT::VMP:
        ...
        if (SG_FORMAT == FORMAT::VMP) {
            offset = 0x80;
        } else if (SG_FORMAT == FORMAT::VGS) {
            offset = 0x40;
        } else if (SG_FORMAT == FORMAT::DEX) {
            offset = 0xF40;
        }
        file.seek(offset);
...
{
    if (SG_HEADER > 0) {
        return QByteArray(reinterpret_cast<char *>(file_headerp), SG_HEADER);
    }
    return QByteArray();
}
...
{
    if (data.size() != SG_HEADER) {
        return false;
    }

    if (SG_HEADER > 0) {
        memcpy(file_headerp, data, uint(SG_HEADER));
    }
    return true;
...
{
    return QByteArray(reinterpret_cast<char *>(&hf[s].sl_header), SG_SLOT_HEADER);
}

bool FF7Save::setSlotHeader(int s, QByteArray data)
{
    if (s < 0 || s > 14 || (data.size() != SG_SLOT_HEADER)) {
        return false;
    }
    memcpy(&hf[s].sl_header, data, uint(SG_SLOT_HEADER));
...
{
    if (SG_FOOTER > 0) {
        return QByteArray(reinterpret_cast<char *>(file_footerp), SG_FOOTER);
    }
    return QByteArray();
}
...
{
    if (data.size() != SG_FOOTER) {
        return false;
    }
    if (SG_FOOTER > 0) {
        memcpy(file_footerp, data, uint(SG_FOOTER));
    }
    return true;
}
...
{
    return QByteArray(reinterpret_cast<char *>(&hf[s].sl_footer), SG_SLOT_FOOTER);
}
bool FF7Save::setSlotFooter(int s, QByteArray data)
{
    if (s < 0 ||  s > 14 || (data.size() != SG_SLOT_FOOTER)) {
        return false;
    }
    memcpy(&hf[s].sl_footer, data, uint(SG_SLOT_FOOTER));
    setFileModified(true, s);
    return true;
}
QByteArray FF7Save::slotPsxRawData(int s)
{
    if (filename.isEmpty() || (format() == FORMAT::PC)) {
        return QByteArray();
    } else if ((format() == FORMAT::PSV) || (format() == FORMAT::PSX))  {
        QFile file(filename);
        if (!file.open(QIODevice::ReadOnly)) {
            return QByteArray();
        }
        QByteArray temp(file.readAll());
        if (format() == FORMAT::PSV) {
            temp.remove(0, 0x84);
        }
        file.close();
        return temp;
...
bool FF7Save::setSlotPsxRawData(int s, QByteArray data)
{
    ...
    for (int i = 0; i < blocks; i++) {
        ...
        offset += 0x0200;
        setSlotFF7Data(s, data.mid(offset, _slotSize));
        offset += _slotSize;
...
bool FF7Save::saveFile(const QString &fileName)
{
    ...
    if (format() == FORMAT::PC) {
        //PC Header Needs to be be Fixed By Host.
        //fix_pc_bytemask(currentSlot) should be called by client to write the last used slot data.
    } else if (format() == FORMAT::PSX) {
        fix_psx_header(0);
    } else if (format() == FORMAT::PSV) {
        fix_psv_header(0);
    } else {
        fix_vmc_header();
    }
    ...
    for (int si = 0; si < SG_SLOT_NUMBER; si++) {
        ...
        file.write(slotFF7Data(si), _slotSize);
...
bool FF7Save::exportFile(const QString &fileName, FF7Save::FORMAT newFormat, int s)
{
    if (fileName.isEmpty()) {
        return false;
    }
    switch (newFormat) {
    case FORMAT::PC: return exportPC(fileName);
    case FORMAT::PSX: return exportPSX(s, fileName);
    case FORMAT::MC: return exportVMC(fileName);
    case FORMAT::VGS: return exportVGS(fileName);
    case FORMAT::DEX: return exportDEX(fileName);
    case FORMAT::UNKNOWN:
    case FORMAT::PSV:
    case FORMAT::VMP:
        break;
...
bool FF7Save::exportPC(const QString &fileName)
{
    ...
    FORMAT prev_type = SG_FORMAT;
    QString prev_fileName = filename;
    if (SG_FORMAT != FORMAT::PC)    {
        for (int i = 0; i < 15; i++) {
            if (isFF7(i)) {
        ...
        // Add File Header
        for (int i = 0; i < 9; i++) {
            file_header_pc[i] = PC_SAVE_GAME_FILE_HEADER[i];
        }
        fix_pc_bytemask(0);//set first slot to 0
    }

    //clean up saves and fix time for Pal Saves.
    for (int i = 0; i < 15; i++) {
        if (isPAL(i)) {
...
bool FF7Save::exportPSX(int s, const QString &fileName)
{
    ...
    if (SG_FORMAT != FORMAT::PSX) {
        ...
        if (fileName.endsWith("S01")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S01, 256);
        } else if (fileName.endsWith("S02")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S02, 256);
        } else if (fileName.endsWith("S03")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S03, 256);
        } else if (fileName.endsWith("S04")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S04, 256);
        } else if (fileName.endsWith("S05")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S05, 256);
        } else if (fileName.endsWith("S06")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S06, 256);
        } else if (fileName.endsWith("S07")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S07, 256);
        } else if (fileName.endsWith("S08")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S08, 256);
        } else if (fileName.endsWith("S09")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S09, 256);
        } else if (fileName.endsWith("S10")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S10, 256);
        } else if (fileName.endsWith("S11")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S10, 256);
        } else if (fileName.endsWith("S12")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S12, 256);
        } else if (fileName.endsWith("S13")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S13, 256);
        } else if (fileName.endsWith("S14")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S14, 256);
        } else if (fileName.endsWith("S15")) {
            memcpy(&hf[s].sl_header, &PSX_SAVE_GAME_FILE_HEADER_S15, 256);
        }
        for (int i = 0; i < SG_SLOT_FOOTER; i++) {
            hf[s].sl_footer[i] = 0x00;
        } //CLEAN FOOTER
    ...
    for (int i = 0; i < blocks; i++) {
        ...
        file.write(slotFF7Data(s), _slotSize);
...
bool FF7Save::exportVMC(const QString &fileName)
{
    ...
    if (SG_FORMAT != FORMAT::MC) {
        ...
        if (prev_type == FORMAT::VMP) {
            for (int i = 0; i < 0x2000; i++) {
                file_headerp[i] = file_header_psp[i + 0x080];
            }
        } else if (prev_type == FORMAT::VGS) {
            for (int i = 0; i < 0x2000; i++) {
                file_headerp[i] = file_header_vgs[i + 0x040];
            }
        } else if (prev_type == FORMAT::DEX) {
            for (int i = 0; i < 0x2000; i++) {
                file_headerp[i] = file_header_dex[i + 0xF40];
            }
        }
    }
    fix_vmc_header();
...
bool FF7Save::exportVGS(const QString &fileName)
{
    ...
    FORMAT prev_type = SG_FORMAT;
    QString prev_fileName = filename;
    if (prev_type != FORMAT::VGS) {
        for (int i = 0; i < 15; i++) {
            if (isFF7(i)) {
                setControlMode(i, CONTROL_NORMAL);
            }
        }
        setFormat(FORMAT::VGS);//fill the Header With The Needed Default
        ...
        //Check if from another kind of VMC and copy the header
        if (prev_type == FORMAT::VMP) {
            for (int i = 0; i < 0x2040; i++) {
                file_headerp[i] = file_header_psp[i + 0x040];
            }
        } else if (prev_type == FORMAT::DEX) {
            for (int i = 0; i < 0x2040; i++) {
                file_headerp[i] = file_header_dex[i + 0xF00];
            }
        } else if (prev_type == FORMAT::MC) {
            for (int i = 0x40; i < 0x2040; i++) {
                file_headerp[i] = file_header_mc[i - 0x40];
            }
        }
    }
    fix_vmc_header();
...
bool FF7Save::exportDEX(const QString &fileName)
{
    ...
    if (SG_FORMAT != FORMAT::DEX) {
        ...
        for (int i = 0x17; i < 0x25; i++) {
            file_header_dex[i] = 0xA0;
        }
        file_header_dex[38] = 0xFF;
        //Check if from another kind of VMC and copy the header
        if (prev_type == FORMAT::MC) {
...
void FF7Save::importSlot(int s, const QString &fileName, int fileSlot)
{
    if (s < 0 || s > 14 || fileName.isEmpty()) {
        return;
    }

    FORMAT inType = FORMAT::UNKNOWN;
    int offset = 0;
    QFile file(fileName);
    if (!file.open(QIODevice::ReadOnly)) {
        return;
    ...
    file.seek(offset);
    setSlotFF7Data(s, file.read(_slotSize));
    /*~~~~~Set Region Data~~~~~~~~~*/
    switch (inType) {
    case FORMAT::PC: {
    ...
    case FORMAT::DEX: {
        ...
        if (inType == FORMAT::VMP) {
            offset = 0x80;
            headerSize = FF7_PSP_SAVE_GAME_HEADER;
        } else if (inType == FORMAT::VGS) {
            offset = 0x40;
            headerSize = FF7_VGS_SAVE_GAME_HEADER;
        } else if (inType == FORMAT::DEX) {
            offset = 0xF40;
            headerSize = FF7_DEX_SAVE_GAME_HEADER;
        }
        file.seek(offset);
        mc_header = file.read(headerSize);
        int index = (128 * fileSlot) + 138;
    ...
    case FORMAT::PSX: {
        if ((file.fileName().contains("00867")) || (file.fileName().contains("00869")) || (file.fileName().contains("00900"))
                || (file.fileName().contains("94163")) || (file.fileName().contains("00700"))
                || (file.fileName().contains("01057")) || (file.fileName().contains("00868"))) {
            QString string;
...
void FF7Save::clearSlot(int rmslot)
{
    ...
    QByteArray temp("\x00", _slotSize);
    setSlotHeader(rmslot, temp.mid(0, SG_SLOT_HEADER));
    setSlotFF7Data(rmslot, temp);
    setSlotFooter(rmslot, temp.mid(0, SG_SLOT_FOOTER));
    SG_Region_String[rmslot].clear();
    if (SG_FORMAT == FORMAT::MC || SG_FORMAT == FORMAT::VMP || SG_FORMAT == FORMAT::VGS || SG_FORMAT == FORMAT::DEX) {
...
{
    memcpy(&slot[s].chars[char_num], new_char.constData(), 132);
    setFileModified(true, s);
}
...
void FF7Save::setItems(int s, QList<quint16> items)
{
    ...
    if (region(s).contains("SLPS-00700")) {
        for (quint16 item : items) {
            if (itemQty(item) > 99 && itemId(item) != 0x1FF) {
                slot[s].items[i] = itemEncode(item, 99);
            } else {
                slot[s].items[i] = item;
            }
            i++;
...
void FF7Save::fix_pc_bytemask(int s)
{
    ...
    default: newheader[4] = quint8(16 * (s - 2)) + 2; break;
    };
...
void FF7Save::fix_psx_header(int s)
{
    ...
    if ((slot[s].time / 3600) > 99) {
        hf[s].sl_header[27] = 0x58; hf[s].sl_header[29] = 0x58;
    } else {
        hf[s].sl_header[27] = quint8((slot[s].time / 3600) / 10) + 0x4F;
        hf[s].sl_header[29] = quint8((slot[s].time / 3600) % 10) + 0x4F;
    }
...
void FF7Save::fix_vmc_header(void)
{
    ...
    if (SG_FORMAT == FORMAT::VMP) {
        for (int i = 0; i < 0x80; i++) {
            mc_header_2.append(char(file_header_psp[i]));
        }
        index = 0x82;
    } else if (SG_FORMAT == FORMAT::VGS) {
        for (int i = 0; i < 0x40; i++) {
            mc_header_2.append(char(file_header_vgs[i]));
        }
        index = 0x42;
    } else if (SG_FORMAT == FORMAT::DEX) {
        for (int i = 0; i < 0xF40; i++) {
            mc_header_2.append(char(file_header_dex[i]));
        }
        index = 0xF42;
    }
    mc_header_2.append(QStringLiteral("MC"));
    if (SG_FORMAT == FORMAT::MC) {
        for (int k = 0; k < 125; k++) {
            mc_header_2.append(char(file_header_mc[k + index]));
        }
    } else if (SG_FORMAT == FORMAT::VMP) {
        for (int k = 0; k < 125; k++) {
            mc_header_2.append(char(file_header_psp[k + index]));
        }
    } else if (SG_FORMAT == FORMAT::VGS) {
        for (int k = 0; k < 125; k++) {
            mc_header_2.append(char(file_header_vgs[k + index]));
        }
    } else if (SG_FORMAT == FORMAT::DEX) {
        for (int k = 0; k < 125; k++) {
            mc_header_2.append(char(file_header_dex[k + index]));
        }
    }
    int x = 0;
    int y = 0;
    ...
    //write xor byte..
    mc_header_2.append(char(xor_byte));
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (isFF7(i)) {
            QByteArray temp = "\x51\x00\x00\x00\x00\x20\x00\x00\xFF\xFF";
            ...
            if (region(i).endsWith("FF7-S01")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S01[i]);
                }
            } else if (region(i).endsWith("FF7-S02")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S02[i]);
                }
            } else if (region(i).endsWith("FF7-S03")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S03[i]);
                }
            } else if (region(i).endsWith("FF7-S04")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S04[i]);
                }
            } else if (region(i).endsWith("FF7-S05")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S05[i]);
                }
            } else if (region(i).endsWith("FF7-S06")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S06[i]);
                }
            } else if (region(i).endsWith("FF7-S07")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S07[i]);
                }
            } else if (region(i).endsWith("FF7-S08")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S08[i]);
                }
            } else if (region(i).endsWith("FF7-S09")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S09[i]);
                }
            } else if (region(i).endsWith("FF7-S10")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S10[i]);
                }
            } else if (region(i).endsWith("FF7-S11")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S11[i]);
                }
            } else if (region(i).endsWith("FF7-S12")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S12[i]);
                }
            } else if (region(i).endsWith("FF7-S13")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S13[i]);
                }
            } else if (region(i).endsWith("FF7-S14")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S14[i]);
                }
            } else if (region(i).endsWith("FF7-S15")) {
                for (int i = 0; i < 256; i++) {
                    temp[i] = char(PSX_SAVE_GAME_FILE_HEADER_S15[i]);
                }
            }
            ...
            if (psx_block_type(i) == 0x52 || psx_block_type(i) == 0x53) {
                if (SG_FORMAT == FORMAT::MC) {
                    for (int j = 0; j < 128; j++) {
                        mc_header_2.append(char(file_header_mc[index + j]));
                    }
                } else if (SG_FORMAT == FORMAT::VMP) {
                    for (int j = 0; j < 128; j++) {
                        mc_header_2.append(char(file_header_psp[index + j]));
                    }
                } else if (SG_FORMAT == FORMAT::VGS) {
                    for (int j = 0; j < 128; j++) {
                        mc_header_2.append(char(file_header_vgs[index + j]));
                    }
                } else if (SG_FORMAT == FORMAT::DEX) {
                    for (int j = 0; j < 128; j++) {
                        mc_header_2.append(char(file_header_dex[index + j]));
                    }
                }
    ...
    if (SG_FORMAT == FORMAT::MC) {
        ...
        for (int i = 0; i < 6143; i++) {
            mc_header_2.append(char(file_header_mc[index + i]));
        }// fill the remainder
        memcpy(file_header_mc, mc_header_2, 0x2000);
    } else if (SG_FORMAT == FORMAT::VMP) {
        index = 2048 + 0x80;
        for (int i = 0; i < 6143; i++) {
            mc_header_2.append(char(file_header_psp[index + i]));
        }// fill the remainder
        ...
        index = 2048 + 0x40;
        for (int i = 0; i < 6143; i++) {
            mc_header_2.append(char(file_header_vgs[index + i]));
        }// fill the remainder
        memcpy(file_header_vgs, mc_header_2, 0x2040);
    } else if (SG_FORMAT == FORMAT::DEX) {
        index = 2048 + 0xF40;
        for (int i = 0; i < 6143; i++) {
            mc_header_2.append(char(file_header_dex[index + i]));
        }// fill the remainder
...
void FF7Save::setSaveNumber(int s, int saveNum)
{
    if (isFF7(s)
            && ((SG_FORMAT == FORMAT::MC)
                || (SG_FORMAT == FORMAT::VMP)
                || (SG_FORMAT == FORMAT::VGS)
                || (SG_FORMAT == FORMAT::DEX)
               )) {
...
{
    if (format() != FORMAT::PC && format() != FORMAT::PSX && format() != FORMAT::PSV) {
        int index = 128 + (128 * s);
        if (format() == FORMAT::VMP) {
            index += 0x80;
        } else if (format() == FORMAT::VGS) {
            index += 0x40;
        } else if (format() == FORMAT::DEX) {
            index += 0xF40;
        }
...
void FF7Save::setPsx_block_next(int s, int next)
{
    ...
    file_headerp[index + 0x08] = quint8(next);
}
...
quint8 FF7Save::psx_block_size(int s)
{
    if (format() == FORMAT::PC) {
        ...
        size -= 0x84;
        quint8 v = quint8(size / 0x2000);
        return v;
    } else if (format() == FORMAT::PSX) {
        return quint8(QFile(fileName()).size() / FF7_PSX_SAVE_GAME_SIZE);
    } else {
        int index = 128 + (128 * s);
        if (format() == FORMAT::VMP) {
            index += 0x80;
...
{
    if (region(s).contains("00867")
            || region(s).contains("00869")
            || region(s).contains("00900")
            || region(s).contains("94163")
            || region(s).contains("00700")
            || region(s).contains("01057")
...
{
    return SG_HEADER;   //Return File Header length
}
int FF7Save::lenFileFooter(void)
{
    return SG_FOOTER;   //Return File Footer length
}
int FF7Save::lenCoreSave(void)
{
    return _slotSize;   //Return Slot length (data portion)
}
int FF7Save::lenSlotHeader(void)
{
    return SG_SLOT_HEADER;   //Return slot header length
}
int FF7Save::lenSlotFooter(void)
{
    return SG_SLOT_FOOTER;   //Return slot footer length
}
int FF7Save::lenSlot(void)
{
    return SG_SLOT_SIZE;   //Return Slot length
}
int FF7Save::numberOfSlots(void)
{
    return SG_SLOT_NUMBER;   //Return number of slots in the file_footer_dex
}
FF7Save::FORMAT FF7Save::format(void)
{
    return SG_FORMAT;   // Returns the file type loaded.
}

void FF7Save::setFormat(FF7Save::FORMAT newFormat)
{
...
void FF7Save::newGame(int s, const QString &fileName)
{
    if (fileName.isEmpty() || fileName.isNull()) {
        memcpy(&slot[s], &default_save, _slotSize);
    } else {
        QFile file(fileName);
        ...
        int index = 0x200;
        temp = ff7file.mid(index, _slotSize);
        memcpy(&slot[s], temp, _slotSize);
    }
    if (isJPN(s)) {
        for (int c = 0; c < 9; c++) {
...
void FF7Save::newGamePlus(int s, const QString &CharFileName, const QString &fileName)
{
    if (fileName.isEmpty() || fileName.isNull())    {
        memcpy(&buffer_slot, &default_save, _slotSize);
    } else {
        ...
        memcpy(&buffer_slot, file.readAll().mid(0x200, _slotSize), _slotSize);
    }
    ...
    // keep all old character info.
    for (int i = 0; i < 9; i++) {
        // except we have to export cait sith and vincent.the game needs y.cloud/seppie, for the flash back.
        // Both cait and Vincent have stats only generated when they join the party.
        if ((i == 6) || (i == 7)) {
            QString outFile = CharFileName;
            if (i == 6) {
                outFile.append("-cait_sith");
                if (format() != FORMAT::PSX || format() != FORMAT::PSV) {
                    outFile = outFile.append("-%1").arg(QString::number(s + 1));
                }
            } else if (i == 7) {
                outFile.append("-vincent");
                if (format() != FORMAT::PSX || format() != FORMAT::PSV) {
                    outFile = outFile.append(QStringLiteral("-%1")).arg(QString::number(s + 1));
                }
            }
            outFile.append(QStringLiteral(".char"));
            exportCharacter(s, i, outFile);
        } else {
            memcpy(&buffer_slot.chars[i], &slot[s].chars[i], 0x84);   // normal character
        }
    }
    memcpy(&buffer_slot.items, &slot[s].items, 640);
    memcpy(&buffer_slot.materias, &slot[s].materias, 800);
    buffer_slot.gil = slot[s].gil;
    buffer_slot.battles = slot[s].battles;
    buffer_slot.runs = slot[s].runs;
    buffer_slot.gp = slot[s].gp;
...
void FF7Save::setMainProgress(int s, int mProgress)
{
    ...
    if (mProgress != slot[s].mprogress) {
        slot[s].mprogress = quint16(mProgress);
        setFileModified(true, s);
    }
}
...
QString FF7Save::charName(int s, int char_num)
{
    ...
    QByteArray text;
    for (int n = 0; n < 12; n++) {
        text.append(char(slot[s].chars[char_num].name[n]));
    }
...
void FF7Save::setCharName(int s, int char_num, QString new_name)
{
    ...
    memcpy(slot[s].chars[char_num].name, temp, size_t(temp.length()));
    setFileModified(true, s);
}
...
QString FF7Save::descName(int s)
{
    ...
    QByteArray text;
    for (int n = 0; n < 16; n++) {
        text.append(char(slot[s].desc.name[n]));
    }
    return Text->toPC(text);
}
void FF7Save::setDescName(int s, QString new_name)
{
    ...
    QByteArray temp = Text->toFF7(new_name);
    memcpy(slot[s].desc.name, temp, size_t(temp.length()));
    setFileModified(true, s);
}
...
QString FF7Save::descLocation(int s)
{
    ...
    QByteArray text;
    for (int n = 0; n < 24; n++) {
        text.append(char(slot[s].desc.location[n]));
    }
    return Text->toPC(text);
}
...
void FF7Save::setDescLocation(int s, QString new_desc_location)
{
    ...
    memcpy(slot[s].desc.location, temp, size_t(temp.length()));
    setFileModified(true, s);
}
...
qint8 FF7Save::descParty(int s, int char_num)
{
...
{
    slot[s].desc.level = quint8(new_level);
    setFileModified(true, s);
}

void FF7Save::setDescParty(int s, int char_num, qint8 new_id)
{
...
QString FF7Save::location(int s)
{
    ...
    QByteArray text;
    for (int n = 0; n < 24; n++) {
        text.append(char(slot[s].location[n]));
    }
    return Text->toPC(text);
}
...
qint8 FF7Save::love(int s, bool battle, FF7Save::LOVER who)
{
    if (battle) {
        switch (who) {
...
void FF7Save::setLove(int s, bool battle, FF7Save::LOVER who, qint8 love)
{
    if (battle) {
        switch (who) {
        case FF7Save::LOVE_BARRET:
            slot[s].b_love.barret = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_TIFA:
            slot[s].b_love.tifa = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_AERIS:
            slot[s].b_love.aeris = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_YUFFIE:
            slot[s].b_love.yuffie = love;
            setFileModified(true, s);
            break;
        }
    } else {
        switch (who) {
        case FF7Save::LOVE_BARRET:
            slot[s].love.barret = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_TIFA:
            slot[s].love.tifa = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_AERIS:
            slot[s].love.aeris = love;
            setFileModified(true, s);
            break;
        case FF7Save::LOVE_YUFFIE:
            slot[s].love.yuffie = love;
            setFileModified(true, s);
            break;
        }
    }
...
void FF7Save::setSpeedScore(int s, int rank, quint16 score)
{
    switch (rank) {
    case 1:
        slot[s].coster_1 = score;
        setFileModified(true, s);
        break;
    case 2:
        slot[s].coster_2 = score;
        setFileModified(true, s);
        break;
    case 3:
        slot[s].coster_3 = score;
        setFileModified(true, s);
        break;
    }
}
...
QString FF7Save::chocoName(int s, int choco_num)
{
    ...
    QByteArray text;
    for (int n = 0; n < 6; n++) {
        text.append(char(slot[s].chocobonames[choco_num][n]));
    }
    return Text->toPC(text);
}
...
void FF7Save::setPartyMateria(int s, int mat_num, quint8 id, qint32 ap)
{
    ...
    if ((id < 91) && ((ap >= 0) && (ap <= 16777215))) {
        ...
        quint8 a = (ap & 0xff);
        quint8 b = (ap & 0xff00) >> 8;
        quint8 c = (ap & 0xff0000) >> 16;
...
{
    slot[s].colors[0][0] = quint8(color.red());
    slot[s].colors[0][1] = quint8(color.green());
    slot[s].colors[0][2] = quint8(color.blue());
    setFileModified(true, s);
}

void FF7Save::setDialogColorUR(int s, QColor color)
{
    slot[s].colors[1][0] = quint8(color.red());
    slot[s].colors[1][1] = quint8(color.green());
    slot[s].colors[1][2] = quint8(color.blue());
    setFileModified(true, s);
}

void FF7Save::setDialogColorLL(int s, QColor color)
{
    slot[s].colors[2][0] = quint8(color.red());
    slot[s].colors[2][1] = quint8(color.green());
    slot[s].colors[2][2] = quint8(color.blue());
    setFileModified(true, s);
}

void FF7Save::setDialogColorLR(int s, QColor color)
{
    slot[s].colors[3][0] = quint8(color.red());
    slot[s].colors[3][1] = quint8(color.green());
    slot[s].colors[3][2] = quint8(color.blue());
    setFileModified(true, s);
}
...
{
    slot[s].chars[char_num].id = quint8(new_id);
    setFileModified(true, s);
}

void FF7Save::setCharLevel(int s, int char_num, qint8 new_level)
{
    slot[s].chars[char_num].level = quint8(new_level);
    setFileModified(true, s);
}
...
void FF7Save::setCharTimeLimitUsed(int s, int char_num, int level, quint16 timesused)
{
    switch (level) {
    case 1:
        slot[s].chars[char_num].timesused1 = timesused;
        setFileModified(true, s);
        break;
    case 2:
        slot[s].chars[char_num].timesused2 = timesused;
        setFileModified(true, s);
        break;
    case 3:
        slot[s].chars[char_num].timesused3 = timesused;
        setFileModified(true, s);
        break;
    }
}
...
void FF7Save::setBattles(int s, int battles)
{
    ...
    slot[s].battles = quint16(battles);
    setFileModified(true, s);
}
...
void FF7Save::setRuns(int s, int runs)
{
    ...
    slot[s].runs = quint16(runs);
    setFileModified(true, s);
}

qint8 FF7Save::party(int s, int pos)
{
...
void FF7Save::setParty(int s, int pos, int new_id)
{
    if (pos >= 0 && pos < 3) {
        if (new_id >= 0 && new_id < 12) {
            slot[s].party[pos] = qint8(new_id);
        } else {
            slot[s].party[pos] = qint8(0xFF);
        }
        setFileModified(true, s);
    }
}
...
void FF7Save::setSnowboardTime(int s, int course, QString value)
{
    switch (course) {
    case 0:
        slot[s].SnowBegFastTime = quint32(value.toInt(nullptr, 16));
        setFileModified(true, s);
        break;
    case 1:
        slot[s].SnowExpFastTime = quint32(value.toInt(nullptr, 16));
        setFileModified(true, s);
        break;
    case 2:
        slot[s].SnowCrazyFastTime = quint32(value.toInt(nullptr, 16));
        setFileModified(true, s);
        break;
    default: break;
    }
...
void FF7Save::setSnowboardScore(int s, int course, quint8 score)
{
    switch (course) {
    case 0:
        slot[s].SnowBegScore = score;
        setFileModified(true, s);
        break;
    case 1:
        slot[s].SnowExpScore = score;
        setFileModified(true, s);
        break;
    case 2:
        slot[s].SnowCrazyScore = score;
        setFileModified(true, s);
        break;
    }
}
...
QByteArray FF7Save::slotFF7Data(int s)
{
    ...
    QByteArray temp;
    temp.setRawData(reinterpret_cast<char *>(&slot[s]), _slotSize);
    return temp;
}
...
bool FF7Save::setSlotFF7Data(int s, QByteArray data)
{
    ...
    if (data.size() != _slotSize) {
        return false;
    }
    memcpy(&slot[s], data, _slotSize);
    setFileModified(true, s);
    return true;
}
...
void FF7Save::setBmProgress1(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        if (value != slot[s].bm_progress1) {
            slot[s].bm_progress1 = quint8(value);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setBmProgress2(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        if (value != slot[s].bm_progress3) {
            slot[s].bm_progress2 = quint8(value);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setBmProgress3(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        value = std::clamp(value, 0, 0xFF);
        if (value != slot[s].bm_progress3) {
            slot[s].bm_progress3 = quint8(value);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setMidgarTrainFlags(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        if (value != slot[s].midgartrainflags) {
            slot[s].midgartrainflags = quint8(value);
            setFileModified(true, s);
        }
    }
}
...
QByteArray FF7Save::unknown(int s, int z)
{
    ...
    if (!(s < 0 || s > 14)) {
        switch (z) {
            ...
            default: temp.setRawData("\x00", 1); break;
        }
    }
    return temp;
}
...
void FF7Save::setBattleSpeed(int s, int speed)
{
    if (speed != slot[s].battlespeed) {
        if (speed < 0 || speed > 255) {
            ...
            slot[s].battlespeed = quint8(speed);
        }
        setFileModified(true, s);
    }
}
...
void FF7Save::setMessageSpeed(int s, int speed)
{
    if (speed != slot[s].fieldmspeed) {
        if (speed < 0 || speed > 255) {
            ...
            slot[s].fieldmspeed = quint8(speed);
        }
        setFileModified(true, s);
    }
}
...
void FF7Save::setBattleMessageSpeed(int s, int speed)
{
    if (speed != slot[s].battlemspeed) {
        if (speed < 0 || speed > 255) {
            ...
            slot[s].battlemspeed = quint8(speed);
        }
        setFileModified(true, s);
    }
}
...
QByteArray FF7Save::controllerMapping(int s)
{
    QByteArray temp;
    for (int i = 0; i < 16; i++) {
        temp.append(char(slot[s].controller_map[i]));
    }
    return temp;
}
...
void FF7Save::setControllerMapping(int s, int action,  int button)
{
    if (button < 0 || button > 15) {
        ...
        if (slot[s].controller_map[action] != button) {
            slot[s].controller_map[action] = quint8(button);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setChocoboPen(int s, int pen, int value)
{
    if (s < 0 || s > 14 || pen < 0 || pen > 3 || value < 0 || value > 8) {
        ...
        slot[s].pennedchocos[pen] = qint8(value);
        setFileModified(true, s);
    }
}
...
quint32 FF7Save::countdownTimer(int s)
{
    if (s < 0 || s > 14) {
        ...
        quint32 seconds = quint32(slot[s].timer[0] | (slot[s].timer[1] << 8) | (slot[s].timer[2] << 16));
        return seconds;
    }
}
...
void FF7Save::setChurchProgress(int s, int progress)
{
    if (s < 0 || s > 14) {
        ...
        if (progress != slot[s].aeris_church) {
            slot[s].aeris_church = quint8(progress);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setDonProgress(int s, int progress)
{
    if (s < 0 || s > 14) {
        ...
        if (progress != slot[s].donprogress) {
            slot[s].donprogress = quint8(progress);
            setFileModified(true, s);
        }
    }
}
...
void FF7Save::setUWeaponHp(int s, int hp)
{
    if (s < 0 || s > 14) {
        ...
        if (quint32(hp) != uWeaponHp(s)) {
            quint8 a = (hp & 0xff);
            quint8 b = (hp & 0xff00) >> 8;
            quint8 c = (hp & 0xff0000) >> 16;
...
qint16 FF7Save::craterSavePointMapID(int s)
{
    if (s < 0 || s > 14) {
        return 0;
    } else {
...
void FF7Save::setCraterSavePointMapID(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        slot[s].cratersaveMapId = qint16(value);
        setFileModified(true, s);
    }
}
...
void FF7Save::setCraterSavePointX(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        slot[s].cratersaveX = qint16(value);
        setFileModified(true, s);
    }
}
...
void FF7Save::setCraterSavePointY(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        slot[s].cratersaveY = qint16(value);
        setFileModified(true, s);
    }
}
...
void FF7Save::setCraterSavePointZ(int s, int value)
{
    if (s < 0 || s > 14) {
        ...
        slot[s].cratersaveZ = qint16(value);
        setFileModified(true, s);
    }
}
...
void FF7Save::setChocoboRating(int s, int stable, int rating)
{
    if (s < 0 || s > 14 || stable < 0 || stable > 5 || rating < 0 || rating > 8) {
        ...
        slot[s].stablechocorating[stable] = quint8(rating);
        setFileModified(true, s);
    }
}
